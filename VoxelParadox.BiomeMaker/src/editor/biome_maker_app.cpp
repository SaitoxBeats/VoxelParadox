// biome_maker_app.cpp
// Unity mental model: Main application loop and UI manager for the Biome Editor.
// Handles file I/O, preset manipulation, ImGui window orchestration, and 3D viewport rendering.
...
#pragma region 1. Constants & Path Helpers
        // --- 1. Constants & Path Helpers ---
        ...
        std::filesystem::path defaultPrivateModuleDirectory(const std::string& presetId) {
            return std::filesystem::path(sanitizePresetId(presetId) + ".modules").lexically_normal();
        }
...
#pragma region 2. UI Utility Helpers
        // --- 2. UI Utility Helpers ---

        template <typename Predicate>
        bool drawGroupedBlockIdOptions(BlockId& targetBlockId, Predicate&& predicate) {
            bool changed = false;
            ...
            auto drawCategory = [&](BlockCategory category) {
                ...
                for (const BlockDefinition& definition : definitions) {
                    ...
                    if (candidate == static_cast<BlockId>(BlockIds::AIR) || !predicate(candidate) ||
                    ...
                    const bool selected = candidate == targetBlockId;
                    if (ImGui::Selectable(definition.displayName.c_str(), selected)) {
                        targetBlockId = candidate;
                        changed = true;
                    }
...
#pragma region 4. Workspace & IO Setup
    // --- 4. Workspace & IO Setup ---
    ...
    void BiomeMakerApp::ensureDefaultModuleLibrary() {
        ...
        for (const DefaultModuleSeed& seed : defaults) {
            const std::filesystem::path path = moduleLibraryDirectory() / (std::string(seed.id) + ".fvmodule.json");
            std::error_code ec;
            ...
            BiomeModule module;
            switch (seed.type) {
            case ModuleType::PERLIN_TERRAIN:      module = BiomeModule::makePerlinTerrain(seed.id, 0); break;
            case ModuleType::IMPORT_VOX_FILES:    module = BiomeModule::makeImportVoxFiles(seed.id); break;
            case ModuleType::GRID_PATTERN:        module = BiomeModule::makeGridPattern(seed.id); break;
            case ModuleType::MENGER_SPONGE:       module = BiomeModule::makeMengerSponge(seed.id); break;
            case ModuleType::CAVE_SYSTEM:         module = BiomeModule::makeCaveSystem(seed.id); break;
            case ModuleType::CELLULAR_NOISE:      module = BiomeModule::makeCellularNoise(seed.id); break;
            case ModuleType::FRACTAL_NOISE:       module = BiomeModule::makeFractalNoise(seed.id); break;
    ...
    void BiomeMakerApp::ensureDefaultPresetExists() {
        ...
        if (!preset.modules.empty()) {
            const std::filesystem::path sharedPerlin = defaultSharedModulePath("perlin.fvmodule.json");
            std::error_code ec;
...
#pragma region 5. Scanning & Document Management
    // --- 5. Scanning & Document Management ---
    ...
    void BiomeMakerApp::scanModuleLibrary() {
        ...
        for (const auto& entry : std::filesystem::directory_iterator(root, ec)) {
            ...
            if (!loadBiomeModuleFromFile(entry.path(), module, error)) {
                appendLog("Skipped module file: " + entry.path().filename().string() + " | " + error);
                continue;
            }

            std::error_code relativeEc;
            std::filesystem::path relativePath = std::filesystem::relative(entry.path(), presetDirectory(), relativeEc);
        ...
        selectedLibraryModuleIndex_ = std::clamp(
            selectedLibraryModuleIndex_, 0, static_cast<int>(moduleLibrary_.size()) - 1
        );
    }
    ...
    void BiomeMakerApp::newPresetDocument() {
        ...
        if (!currentPreset_.modules.empty()) {
            ...
            if (std::filesystem::exists(resolvePresetRelativePath(sharedPerlin), ec)) {
                ...
                currentPreset_.modules.front().displayName = displayModuleName(currentPreset_.modules.front());
            }
        }
    ...
    void BiomeMakerApp::duplicatePresetDocument() {
        ...
        for (int index = 0; index < static_cast<int>(currentPreset_.modules.size()); index++) {
            ...
            std::string error;
            if (!saveBiomeModuleToFile(resolvePresetRelativePath(module.filePath), module, error)) {
                appendLog("Failed to copy module file: " + module.filePath.generic_string() + " | " + error);
            }
        }
...
#pragma region 6. State & Buffers
    ...
    void BiomeMakerApp::syncPreviewSource() {
        if (!previewSourceDirty_ || ImGui::IsAnyItemActive()) {
            return;
        }
        ...
        previewWorld_.setGeneratorSource(std::make_shared<PresetModuleGeneratorSource>(currentPreset_));
    ...
    void BiomeMakerApp::handleShortcuts() {
        ...
        const bool ctrlDown = Input::keyDown(GLFW_KEY_LEFT_CONTROL) || Input::keyDown(GLFW_KEY_RIGHT_CONTROL);
        const bool shiftDown = Input::keyDown(GLFW_KEY_LEFT_SHIFT) || Input::keyDown(GLFW_KEY_RIGHT_SHIFT);
...
#pragma region 7. Path Resolution
    // --- 7. Path Resolution ---

    std::filesystem::path BiomeMakerApp::defaultPathForPreset(const BiomePreset& preset) const {
        return presetRootDirectory() / (sanitizePresetId(preset.id) + ".fvbiome.json");
    }

    std::filesystem::path BiomeMakerApp::presetDirectory() const {
        const std::filesystem::path presetPath = currentPresetPath_.empty()
            ? defaultPathForPreset(currentPreset_)
            : currentPresetPath_;
        return presetPath.parent_path();
    }

    std::filesystem::path BiomeMakerApp::resolvePresetRelativePath(const std::filesystem::path& relativePath) const {
    ...
    std::filesystem::path BiomeMakerApp::makeUniqueModuleRelativePath(
        ...
        const std::filesystem::path privateModuleDirectory = defaultPrivateModuleDirectory(currentPreset_.id);

        for (int suffix = 1;; suffix++) {
            const std::string fileName = suffix == 1
                ? sanitizedBase + ".fvmodule.json"
                : sanitizedBase + "_" + std::to_string(suffix) + ".fvmodule.json";

            const std::filesystem::path candidate = (privateModuleDirectory / fileName).lexically_normal();
    ...
    bool BiomeMakerApp::isSharedLibraryModulePath(const std::filesystem::path& path) const {
        if (path.empty()) {
            return false;
        }
        ...
        if (path.is_absolute()) {
            std::error_code ec;
            const std::filesystem::path canonicalLibraryDir = std::filesystem::weakly_canonical(moduleLibraryDirectory(), ec);
            if (ec) {
                return false;
            }
...
#pragma region 8. Module Selection & Editing
    // --- 8. Module Selection & Editing ---
    ...
    void BiomeMakerApp::clampSelectedModuleIndex() {
        ...
        selectedModuleIndex_ = std::clamp(
            selectedModuleIndex_, 0, static_cast<int>(currentPreset_.modules.size()) - 1
        );
    }
    ...
    std::string BiomeMakerApp::makeUniqueModuleId(const std::string& base, int ignoreIndex) const {
    ...
    void BiomeMakerApp::duplicateSelectedModule() {
        ...
        if (!saveBiomeModuleToFile(resolvePresetRelativePath(copy.filePath), copy, error)) {
            appendLog("Failed to duplicate module file: " + copy.filePath.generic_string() + " | " + error);
            return;
        }
    ...
    void BiomeMakerApp::removeSelectedModule() {
        if (selectedModuleIndex_ < 0 || selectedModuleIndex_ >= static_cast<int>(currentPreset_.modules.size())) {
            return;
        }

        currentPreset_.modules.erase(currentPreset_.modules.begin() + selectedModuleIndex_);
    ...
    void BiomeMakerApp::moveSelectedModule(int direction) {
        ...
        if (selectedModuleIndex_ < 0 || otherIndex < 0 || otherIndex >= static_cast<int>(currentPreset_.modules.size())) {
            return;
        }

        std::swap(
            currentPreset_.modules[static_cast<std::size_t>(selectedModuleIndex_)],
...
#pragma region 9. Main UI Drawing
    // --- 9. Main UI Drawing ---
    ...
    void BiomeMakerApp::drawDockspace() {
        ...
        const ImGuiID dockspaceId = ImGui::DockSpaceOverViewport(0, viewport, ImGuiDockNodeFlags_None);
        ...
        ImGui::DockBuilderSplitNode(centerNode, ImGuiDir_Right, 0.23f, &rightNode, &centerNode);
    ...
    void BiomeMakerApp::drawMenuBar() {
        ...
        if (ImGui::BeginMenu("File")) {
            ...
            if (ImGui::MenuItem("Revert to Disk", nullptr, false, canSave && !currentPresetPath_.empty())) {
...
#pragma region 10. UI: Browsers & Stats
    // --- 10. UI: Browsers & Stats ---
    ...
    void BiomeMakerApp::drawStatsWindow() {
        ...
        ImGui::Separator();
        ImGui::Text("Revision: %llu", static_cast<unsigned long long>(stats.generationRevision));
...
#pragma region 11. UI: Inspector
    ...
    void BiomeMakerApp::drawInspector() {
        ...
        if (ImGui::Checkbox("Enable Fog", &viewportFogEnabled_)) {
            appendLog(viewportFogEnabled_ ? "Enabled viewport fog." : "Disabled viewport fog.");
        }
        ImGui::Separator();

        // --- 2. Preset Identification ---
        ...
        const int currentModeIndex = currentPreset_.preview.defaultMode == PreviewMode::SANDBOX ? 0 : 1;
        int modeIndex = currentModeIndex;
        const char* modes[] = { "Sandbox", "Streaming" };

        if (ImGui::Combo("Default Mode", &modeIndex, modes, 2)) {
            currentPreset_.preview.defaultMode = modeIndex == 0 ? PreviewMode::SANDBOX : PreviewMode::STREAMING;
            markDocumentDirty("Updated preset preview mode.");
        }

        std::array<int, 3> sandboxRadius = {
            currentPreset_.preview.sandboxRadius.x,
        ...
        if (ImGui::InputInt3("Sandbox Radius", sandboxRadius.data())) {
            currentPreset_.preview.sandboxRadius = glm::max(
                glm::ivec3(sandboxRadius[0], sandboxRadius[1], sandboxRadius[2]),
                glm::ivec3(0)
            );
        ...
        for (int index = 0; index < static_cast<int>(currentPreset_.modules.size()); index++) {
            const BiomeModule& module = currentPreset_.modules[static_cast<std::size_t>(index)];
            const bool selected = index == selectedModuleIndex_;
            const std::string label = std::to_string(index + 1) + ". " + displayModuleName(module) + "##" + module.id;
        ...
        if (moduleLibrary_.empty()) {
            ...
            const ModuleLibraryEntry& selectedLibraryModule = moduleLibrary_[static_cast<std::size_t>(selectedLibraryModuleIndex_)];

            if (ImGui::BeginCombo("Available Modules", displayModuleName(selectedLibraryModule.module).c_str())) {
                for (int index = 0; index < static_cast<int>(moduleLibrary_.size()); index++) {
                    const ModuleLibraryEntry& entry = moduleLibrary_[static_cast<std::size_t>(index)];
                    const bool selected = index == selectedLibraryModuleIndex_;
                    const std::string label = displayModuleName(entry.module) + "##library_" + std::to_string(index);
            ...
            ImGui::TextDisabled("Selected File: %s", selectedLibraryModule.relativePath.generic_string().c_str());
        ...
        if (BiomeModule* module = selectedModule()) {
            ...
            const std::filesystem::path modulePath = module->filePath.empty()
                ? makeUniqueModuleRelativePath(module->id)
                : module->filePath;
            const std::filesystem::path resolvedModulePath = resolvePresetRelativePath(modulePath);

            if (module->filePath.empty()) {
                ImGui::TextDisabled("Module File (created on save): %s", modulePath.generic_string().c_str());
            }
            else if (std::filesystem::exists(resolvedModulePath)) {
                ImGui::TextDisabled("Module File: %s", modulePath.generic_string().c_str());
            }
            else {
                ImGui::TextColored(ImVec4(0.95f, 0.75f, 0.35f, 1.0f), "Module File Missing: %s", modulePath.generic_string().c_str());
            }

            // Shared drawing lambdas
            const auto drawTricolorPalette = [&](TricolorPaletteModule& palette) {
                ImGui::SeparatorText("Voxel Palette");
                if (drawBlockIdCombo("Primary", palette.primary)) {
                    markSelectedModuleDirty("Updated primary palette block.");
                }
                if (drawBlockIdCombo("Secondary", palette.secondary)) {
                    markSelectedModuleDirty("Updated secondary palette block.");
                }
                if (drawBlockIdCombo("Accent", palette.accent)) {
            ...
            const auto drawVolumeNoiseSettings = [&](VolumeNoiseModuleSettings& noise) {
                ...
                if (ImGui::SliderFloat("Base Scale", &noise.baseScale, 0.001f, 0.25f, "%.4f", ImGuiSliderFlags_Logarithmic)) {
                ...
                if (ImGui::SliderFloat("Persistence", &noise.persistence, 0.05f, 0.95f, "%.3f")) {
                    markSelectedModuleDirty("Updated noise persistence.");
                }
                if (ImGui::SliderFloat("Lacunarity", &noise.lacunarity, 1.05f, 6.0f, "%.3f")) {
                    markSelectedModuleDirty("Updated noise lacunarity.");
                }
                if (ImGui::SliderFloat("Threshold", &noise.threshold, -1.0f, 1.0f, "%.3f")) {
                    markSelectedModuleDirty("Updated noise threshold.");
                }
                if (ImGui::Checkbox("Invert Pattern", &noise.invert)) {
                    markSelectedModuleDirty(noise.invert ? "Enabled noise inversion." : "Disabled noise inversion.");
                }

                if (ImGui::Checkbox("Infinite Y", &noise.infiniteY)) {
                    markSelectedModuleDirty(noise.infiniteY ? "Enabled infinite Y range." : "Disabled infinite Y range.");
                }
                ...
                if (ImGui::SliderFloat("Secondary Noise Scale", &noise.secondaryNoiseScale, 0.0f, 0.25f, "%.3f")) {
                    markSelectedModuleDirty("Updated secondary noise scale.");
                }
                if (ImGui::SliderFloat("Secondary Threshold", &noise.secondaryThreshold, -1.0f, 1.0f, "%.3f")) {
                    markSelectedModuleDirty("Updated secondary threshold.");
                }
                if (ImGui::SliderFloat("Accent Noise Scale", &noise.accentNoiseScale, 0.0f, 0.25f, "%.3f")) {
                    markSelectedModuleDirty("Updated accent noise scale.");
                }
                if (ImGui::SliderFloat("Accent Threshold", &noise.accentThreshold, -1.0f, 1.0f, "%.3f")) {
            ...
            if (module->type == ModuleType::PERLIN_TERRAIN) {
                ...
                if (ImGui::SliderFloat("Base Scale", &density.baseScale, 0.005f, 0.20f, "%.3f")) {
                    markSelectedModuleDirty("Updated base scale.");
                }
                if (ImGui::SliderFloat("Density Threshold", &density.densityThreshold, -0.20f, 0.80f, "%.3f")) {
                ...
                if (ImGui::SliderFloat("Density Persistence", &density.densityPersistence, 0.10f, 0.95f, "%.3f")) {
                ...
                if (ImGui::InputText("Source Directory", moduleSourceDirBuffer_, sizeof(moduleSourceDirBuffer_))) {
                ...
                if (ImGui::Checkbox("Infinite Y", &vox.infiniteY)) {
                    markSelectedModuleDirty(vox.infiniteY ? "Enabled infinite VOX Y placement." : "Disabled infinite VOX Y placement.");
                }
                ...
                if (ImGui::SliderFloat("Spawn Chance", &vox.spawnChance, 0.0f, 1.0f, "%.2f")) {
                ...
                if (drawBlockIdCombo("Default Voxel", vox.defaultVoxel)) {
                ...
                if (ImGui::InputInt3("Cell Size", cellSize.data())) {
                    grid.cellSize = glm::max(glm::ivec3(cellSize[0], cellSize[1], cellSize[2]), glm::ivec3(1));
                    grid.lineWidth = glm::min(grid.lineWidth, grid.cellSize);
                    markSelectedModuleDirty("Updated grid cell size.");
                }

                std::array<int, 3> lineWidth = { grid.lineWidth.x, grid.lineWidth.y, grid.lineWidth.z };
                if (ImGui::InputInt3("Line Width", lineWidth.data())) {
                    grid.lineWidth = glm::min(glm::max(glm::ivec3(lineWidth[0], lineWidth[1], lineWidth[2]), glm::ivec3(1)), grid.cellSize);
                ...
                if (ImGui::SliderInt("Max Axis Matches", &grid.maxAxisMatches, grid.minAxisMatches, 3)) {
                    markSelectedModuleDirty("Updated grid maximum axis matches.");
                }

                if (ImGui::Checkbox("Infinite Y", &grid.infiniteY)) {
                    markSelectedModuleDirty(grid.infiniteY ? "Enabled infinite grid Y." : "Disabled infinite grid Y.");
                }
                ...
                if (ImGui::SliderFloat("Warp Strength", &grid.warpStrength, 0.0f, 16.0f, "%.2f")) {
                    markSelectedModuleDirty("Updated grid warp strength.");
                }
                if (ImGui::SliderFloat("Accent Noise Scale", &grid.accentNoiseScale, 0.0f, 0.25f, "%.3f")) {
                    markSelectedModuleDirty("Updated grid accent noise scale.");
                }
                if (ImGui::SliderFloat("Accent Threshold", &grid.accentThreshold, -1.0f, 1.0f, "%.3f")) {
                ...
                std::array<int, 3> cellSize = { sponge.cellSize.x, sponge.cellSize.y, sponge.cellSize.z };
                if (ImGui::InputInt3("Cell Size", cellSize.data())) {
                    sponge.cellSize = glm::max(glm::ivec3(cellSize[0], cellSize[1], cellSize[2]), glm::ivec3(3));
                ...
                if (ImGui::Checkbox("Invert Pattern", &sponge.invert)) {
                    markSelectedModuleDirty(sponge.invert ? "Enabled sponge inversion." : "Disabled sponge inversion.");
                }

                if (ImGui::Checkbox("Infinite Y", &sponge.infiniteY)) {
                    markSelectedModuleDirty(sponge.infiniteY ? "Enabled infinite sponge Y." : "Disabled infinite sponge Y.");
                }
                ...
                if (ImGui::SliderFloat("Warp Scale", &sponge.warpScale, 0.0f, 0.20f, "%.3f")) {
                    markSelectedModuleDirty("Updated sponge warp scale.");
                }
                if (ImGui::SliderFloat("Warp Strength", &sponge.warpStrength, 0.0f, 12.0f, "%.2f")) {
                    markSelectedModuleDirty("Updated sponge warp strength.");
                }
                if (ImGui::SliderFloat("Accent Noise Scale", &sponge.accentNoiseScale, 0.0f, 0.25f, "%.3f")) {
                    markSelectedModuleDirty("Updated sponge accent noise scale.");
                }
                if (ImGui::SliderFloat("Accent Threshold", &sponge.accentThreshold, -1.0f, 1.0f, "%.3f")) {
                ...
                std::array<int, 3> rootCellSize = { caves.rootCellSize.x, caves.rootCellSize.y, caves.rootCellSize.z };
                if (ImGui::InputInt3("Root Cell Size", rootCellSize.data())) {
                    caves.rootCellSize = glm::max(glm::ivec3(rootCellSize[0], rootCellSize[1], rootCellSize[2]), glm::ivec3(4));
                ...
                if (ImGui::SliderFloat("Tunnel Radius", &caves.tunnelRadius, 0.5f, 24.0f, "%.2f")) {
                    markSelectedModuleDirty("Updated cave tunnel radius.");
                }
                if (ImGui::SliderFloat("Radius Falloff", &caves.radiusFalloff, 0.15f, 0.95f, "%.2f")) {
                    markSelectedModuleDirty("Updated cave radius falloff.");
                }
                if (ImGui::SliderFloat("Chamber Chance", &caves.chamberChance, 0.0f, 1.0f, "%.2f")) {
                    markSelectedModuleDirty("Updated cave chamber chance.");
                }
                if (ImGui::SliderFloat("Chamber Radius Scale", &caves.chamberRadiusScale, 0.5f, 4.0f, "%.2f")) {
                    markSelectedModuleDirty("Updated cave chamber radius scale.");
                }
                if (ImGui::SliderFloat("Center Jitter", &caves.centerJitter, 0.0f, 0.45f, "%.2f")) {
                    markSelectedModuleDirty("Updated cave center jitter.");
                }

                if (ImGui::Checkbox("Infinite Y", &caves.infiniteY)) {
                    markSelectedModuleDirty(caves.infiniteY ? "Enabled infinite cave Y." : "Disabled infinite cave Y.");
                }
                ...
                if (ImGui::SliderFloat("Warp Scale", &caves.warpScale, 0.0f, 0.15f, "%.3f")) {
                    markSelectedModuleDirty("Updated cave warp scale.");
                }
                if (ImGui::SliderFloat("Warp Strength", &caves.warpStrength, 0.0f, 20.0f, "%.2f")) {
                    markSelectedModuleDirty("Updated cave warp strength.");
                }
                if (ImGui::SliderFloat("Accent Noise Scale", &caves.accentNoiseScale, 0.0f, 0.25f, "%.3f")) {
                    markSelectedModuleDirty("Updated cave accent noise scale.");
                }
                if (ImGui::SliderFloat("Accent Threshold", &caves.accentThreshold, -1.0f, 1.0f, "%.3f")) {
                ...
                if (ImGui::SliderFloat("Distance Blend", &cellular.distanceBlend, 0.0f, 1.0f, "%.3f")) {
                ...
                if (ImGui::SliderFloat("Ridge Sharpness", &ridged.ridgeSharpness, 0.05f, 8.0f, "%.3f", ImGuiSliderFlags_Logarithmic)) {
                ...
                if (ImGui::SliderFloat("Warp Persistence", &warped.warpPersistence, 0.05f, 0.95f, "%.3f")) {
                    markSelectedModuleDirty("Updated warp persistence.");
                }
                if (ImGui::SliderFloat("Warp Lacunarity", &warped.warpLacunarity, 1.05f, 6.0f, "%.3f")) {
                    markSelectedModuleDirty("Updated warp lacunarity.");
                }
                if (ImGui::SliderFloat("Warp Scale", &warped.warpScale, 0.0f, 0.25f, "%.4f")) {
                    markSelectedModuleDirty("Updated warp scale.");
                }
                if (ImGui::SliderFloat("Warp Strength", &warped.warpStrength, 0.0f, 24.0f, "%.3f")) {
                    markSelectedModuleDirty("Updated warp strength.");
                }
                ...
                if (drawBlockIdMultiSelect("Spawn On Blocks", tree.spawnOnBlocks)) {
                ...
                if (ImGui::BeginCombo("Tree Type", treeGeneratorTypeDisplayName(tree.treeType))) {
...
#pragma region 12. UI: Popups & Dialogs
    // --- 12. UI: Popups & Dialogs ---
    ...
    void BiomeMakerApp::drawOpenPresetPopup() {
        ...
        if (!ImGui::BeginPopupModal("Open Preset", nullptr, ImGuiWindowFlags_AlwaysAutoResize)) {
            return;
        }
    ...
    bool BiomeMakerApp::drawBlockIdMultiSelect(const char* label, std::vector<BlockId>& blockIds) {
        ...
        if (!blockIds.empty()) {
            ...
            for (std::size_t index = 0; index < blockIds.size(); ++index) {
                ...
                preview += getBlockDisplayName(blockIds[index]);
                if (index == 2 && blockIds.size() > 3) {
                    preview += ", ...";
                    break;
                }
            }
        ...
        if (ImGui::BeginCombo(label, preview.c_str())) {
            ...
            auto drawCategory = [&](BlockCategory category) {
                ...
                for (const BlockDefinition& definition : definitions) {
                    ...
                    const auto iterator = std::find(blockIds.begin(), blockIds.end(), candidate);
                    const bool selected = iterator != blockIds.end();

                    if (ImGui::Selectable(definition.displayName.c_str(), selected, ImGuiSelectableFlags_DontClosePopups)) {
                        if (selected) {
                            if (blockIds.size() > 1) {
                                blockIds.erase(iterator);
                                changed = true;
                            }
                        }
                        else {
                            blockIds.push_back(candidate);
                            std::sort(blockIds.begin(), blockIds.end(),
                                [](BlockId left, BlockId right) { return left < right; });
                            changed = true;
                        }
                    }
