// Arquivo: VoxelParadox.Client/src/World/world/generation/noise.hpp
...
inline int fastFloor(float value) {
...
inline float perlin(float x, float y, float z) {
    const int floorX = fastFloor(x);
    const int floorY = fastFloor(y);
    const int floorZ = fastFloor(z);
    int X = floorX & 255;
    int Y = floorY & 255;
    int Z = floorZ & 255;
